...
def CheckURI(uri, timeout=5):
    try:
        r = str(requests.get(uri,timeout=5.0))
        if r == "<Response [200]>":
            return True
        else:
            return False
...
class Shortener(http.server.BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if "longuri" not in params or "shortname" not in params:
            ...
            self.send_header('Content-type', 'text/plain; charset=utf-8')
            self.end_headers()
            self.wfile.write("ERROR - Please ensure that both fields in the form have been filled in.".encode())
        ...
        if CheckURI(longuri):
            ...
            self.send_header('Content-type', 'text/plain; charset=utf-8')
            self.end_headers()
            self.wfile.write("ERROR - The URI was not successfully fetched".encode())
